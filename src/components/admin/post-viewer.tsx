 ...
 * 集成 Fumadocs 的强大渲染能力
 */
export function PostViewer({ post, isOpen, onClose, initialMode = 'simple' }: PostViewerProps) {
  ...
  // 渲染内容
  const renderContent = (isPreview = false) => {
    if (loading) {
      return (
  ...
  return (
    ...
    >
      <DialogContent className={`
        ...
        flex flex-col
      `}>
        {viewMode === 'simple' ? renderSimpleMode() : renderPreviewMode()}
      </DialogContent>
    </Dialog>
  );
}